...
def test(data,
    ...
    if save_json and len(jdict):
        w = Path(weights[0] if isinstance(weights, list) else weights).stem if weights is not None else ''  # model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', nargs='+', type=str, default='yolov5s.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='data/plate_color.yaml', help='*.data path')
