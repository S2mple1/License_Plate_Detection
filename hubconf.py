...
def create(name, pretrained, channels, classes, autoshape):
    ...
    Arguments:
        ...
        pretrained (bool): load pretrained model into the model
...
def yolov5s(pretrained=False, channels=3, classes=80, autoshape=True):
    ...
    Arguments:
        pretrained (bool): load pretrained model into the model, default=False
...
if __name__ == '__main__':
    ...
    imgs = [Image.open(x) for x in Path('imgs/').glob('*.jpg')]
    results = model(imgs)
    results.show()
    results.print()
